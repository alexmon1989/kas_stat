<template>
    <div>
        ...
        <b-table-simple small caption-top responsive striped bordered v-else>
            <caption>Статистика навантаження фахівців за період від {{ dateFrom | moment("DD.MM.YYYY") }} до {{ dateTo | moment("DD.MM.YYYY") }}</caption>
            <b-thead class="text-center">
                <b-tr>
                    ...
                    <b-th rowspan="2">Кількість виданих дублікатів</b-th>
            ...
            <b-tbody class="text-center">
                <b-tr v-for="item in statistics" :key="item.name">
                    ...
                    <b-td>{{ item.duplicates }}</b-td>
                    <b-td>{{ item.changes }}</b-td>
                    <b-td>{{ item.budget_preparing }}</b-td>
                    <b-td>{{ item.budget_issue }}</b-td>
                    <b-td>{{ item.budget_duplicate }}</b-td>
                ...
                <b-tr>
                    ...
                    <b-td>{{ duplicates_total }}</b-td>
                    <b-td>{{ changes_total }}</b-td>
                    <b-td>{{ budget_preparing_total }}</b-td>
                    <b-td>{{ budget_issue_total }}</b-td>
                    <b-td>{{ budget_duplicate_total }}</b-td>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            budget_issue_total() {
                return this.statistics.reduce(function (sum, current) {
                    return sum + current.budget_issue;
                }, 0);
            },

            budget_duplicate_total() {
                return this.statistics.reduce(function (sum, current) {
                    return sum + current.budget_duplicate;
                }, 0);
            },
        ...
        methods: {
            getStatisticsData: function () {
                this.isBusy = true;
                axios.get('/api/statistics/ap_specialist_workload_statistics/', {
                    params: {
